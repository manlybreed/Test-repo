...
import { getCandidateMeetingSlots } from "@/lib/calendar/propose-times";
...
/**
 * Shared system-prompt rule for real-calendar-grounded meeting-time
 * options. availableSlots is generated by code (generateCandidateSlots in
 * propose-times.ts), never by this model call — so the instruction here
 * is deliberately narrow: use them verbatim if relevant, or ignore the
 * field entirely, never extend/reword the list into new times.
 */
function availabilityPromptRule(availableSlots: string[]): string {
  return availableSlots.length
    ? `availableSlots in mail_data lists real open times on the sender's calendar, already confirmed against actual availability. If (and only if) this reply is proposing or discussing scheduling a meeting/call, you may offer 2-3 of these exact slots verbatim as options. Never invent a time not in this list, and never mention it at all if scheduling isn't relevant here.`
    : `availableSlots in mail_data is empty — if this reply needs to propose a time, ask the recipient for their availability instead of inventing a specific time.`;
}
...
export async function draftReply(opts: {
  ...
  // Best-effort: silently offers nothing if Calendar isn't connected for
  // this account, rather than failing the whole draft over it.
  const availableSlots = await getCandidateMeetingSlots(opts.accountId)
    .then((slots) => slots.map((s) => s.label))
    .catch(() => [] as string[]);
...
${availabilityPromptRule(availableSlots)}`,
    user: `${fenceMailData({ threadContext: packed, attachedDocuments: attachedDocuments.length ? attachedDocuments : null, availableSlots: availableSlots.length ? availableSlots : null })}\n\nIntent: ${opts.intent || "reply appropriately"}\nTone: ${tone}`,
  });
