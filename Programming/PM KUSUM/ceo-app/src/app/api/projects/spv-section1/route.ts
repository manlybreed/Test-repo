...
/** Fill disclosure Section 1 from SPV KYC (skips if already done unless force). */
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        send({
          pct: 2,
          step: force ? "Force re-run Section 1…" : "Starting Section 1 (SPV KYC)…",
        });
        const result = await runSpvSection1Fill(
          plantId,
          async (pct, step) => {
