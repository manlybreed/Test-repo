...
/** Checkpoint 1 — Land KYC khasra verification (no disclosure form fill). */
export async function POST(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        send({ pct: 2, step: "Starting Land KYC checkpoint…" });
        const result = await runLandKycCheck(plantId, async (pct, step) => {
          send({ pct, step });
        });
