...
import { InvoiceGstr1Export } from "@/components/invoice-gstr1-export";
import { InvoiceAskPanel } from "@/components/invoice-ask-panel";
import { formatINR } from "@/lib/utils";

export default async function InvoicesPage({
  searchParams,
}: {
  searchParams: Promise<{ created?: string; file?: string }>;
}) {
  ...
  const totalTaxable = taxInvoices.reduce((s, i) => s + i.taxableTotal, 0);
  const totalGrand = taxInvoices.reduce((s, i) => s + i.grandTotal, 0);
  const totalTax = totalGrand - totalTaxable;

  const clientEmailById = new Map(clients.map((c) => [c.id, c.email ?? ""]));
  const dbRows = invoices.map((inv) => ({
    ...
    clientEmail: inv.clientId ? clientEmailById.get(inv.clientId) || null : null,
  }));

  const fileHref = sp.file ? `/api/files/${sp.file}` : null;
    ...
    <div>
      <header className="mb-8">
        ...
        <p className="text-sm max-w-xl" style={{ color: "var(--text-muted)" }}>
          GST-first documents (tax invoice, proforma, credit notes) with AI draft,
          validation, and GSTR-1 export. Issued docs are immutable.
        </p>
      </header>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          ...
          >
            ...
            <p className="relative text-xl font-bold tabular-nums leading-tight" style={{ color: s.color }}>
              {s.value}
            </p>
          </div>
        ))}
      ...
      {sp.created && (
        ...
        >
          <span className="flex items-center gap-2" style={{ color: "#34d399" }}>
            <span>✓</span> Document {decodeURIComponent(sp.created)} issued.
          </span>
          {fileHref && (
      ...
      >
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse 50% 60% at 0% 0%, rgba(240,180,41,0.06) 0%, transparent 55%)",
          }}
        />
        <div className="relative p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-base font-semibold">New tax / proforma invoice</h2>
              <p className="text-xs mt-0.5" style={{ color: "var(--text-dim)" }}>
                Pick Proforma at the top of the form if you will bill later. Convert with → Tax inv in the table when ready.
              </p>
            </div>
          </div>
